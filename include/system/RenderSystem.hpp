    ...
    void render() {
        ...
        for (Entity e : world->getEntities()) {
            ...
            if (tc && rc && rc->model) {
                ...
                modelMat = glm::translate(modelMat, glm::vec3(tc->position[0], tc->position[1], tc->position[2]));
                modelMat = glm::rotate(modelMat, glm::radians(tc->rotation[0]), glm::vec3(1,0,0));
                modelMat = glm::rotate(modelMat, glm::radians(tc->rotation[1]), glm::vec3(0,1,0));
                modelMat = glm::rotate(modelMat, glm::radians(tc->rotation[2]), glm::vec3(0,0,1));
                modelMat = glm::scale(modelMat, glm::vec3(tc->scale[0], tc->scale[1], tc->scale[2]));
...
private:
    ...
    void uploadModelToGPU(Model* model) {
        ...
        vertexData.reserve(vertCount * (3 + (hasNormals?3:0) + (hasTexcoords?2:0)));
        ...
        GLsizei stride = (GLsizei)((3 + (hasNormals?3:0) + (hasTexcoords?2:0)) * sizeof(float));
        size_t offset = 0;
