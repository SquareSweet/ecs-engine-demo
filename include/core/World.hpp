...
public:
  ...
  bool hasRender(Entity e) const { return renders.find(e) != renders.end(); }
  bool hasScript(Entity e) const { return scripts.find(e) != scripts.end(); }
  ...
  // For serialization
  const std::unordered_map<Entity, TransformComponent> &
  getAllTransforms() const {
    return transforms;
  }
