...
bool ResourceManager::parseOBJ(const std::string& path, Model& outModel) {
    ...
    for (const auto& mat : materials) {
        ...
        mi.name = mat.name;
        mi.diffuse_color = glm::vec3(mat.diffuse[0], mat.diffuse[1], mat.diffuse[2]);
        mi.ambient_color = glm::vec3(mat.ambient[0], mat.ambient[1], mat.ambient[2]);
        mi.specular_color= glm::vec3(mat.specular[0], mat.specular[1], mat.specular[2]);
    ...
    for (const auto& shape : shapes) {
        ...
        for (size_t idx = 0; idx < mesh.indices.size(); ++idx) {
            ...
            if (it != uniqueVertexMap.end()) {
                ...
                unsigned int newIndex = static_cast<unsigned int>(outModel.positions.size() / 3); // текущий count вершин
    ...
    if (!hasNormals) {
        ...
        for (size_t vi = 0; vi < vertCount; ++vi) {
            glm::vec3 n(
                outModel.normals[3*vi + 0],
                outModel.normals[3*vi + 1],
